...
void PPGService::OnNewPPGValue(uint16_t hrs, uint16_t als, int16_t x, int16_t y, int16_t z, uint16_t count) {
  if (!ppgValueNotificationEnabled)
    return;
  uint16_t buf[6] = {hrs, als, static_cast<uint16_t>(x), static_cast<uint16_t>(y), static_cast<uint16_t>(z), count};
